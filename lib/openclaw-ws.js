...
function ensureRequest() {
  ...
  // Gateway protocol v4 doesn't always emit stream=user for the dashboard,
  // so assistant runs would have no request to attach to. Create a synthetic
  // one so activity still appears in the dashboard (content backfilled from
  // lastUserMessage if available).
  const synthId = `req_${Date.now()}`
  const synthContent = (lastUserMessage || 'Activity from assistant').slice(0, 200)
  createRequest({
    id: synthId,
    ...
    task: null,
    createdAt: Date.now(),
    source: 'assistant_synth',
  })
  incrementMessages('received')
  currentRequestId = synthId
  console.log(`[openclaw-ws] Created synthetic request: ${synthId}`)
  const synthMsg = lastUserMessage
    ? `📥 Activity: "${synthContent.slice(0, 50)}${synthContent.length > 50 ? '...' : ''}"`
    : '📥 Activity from assistant'
  createDashboardEvent(synthId, 'received', 'wickedman', synthMsg)
  return synthId
}
